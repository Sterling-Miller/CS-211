...
class Board(object):
    ...
    def __init__(self):
        ...
        for col in range(len(self.tiles[0])):
            ...
            for row in range(len(self.tiles)):
                ttl.append(self.tiles[row][col])
            self.groups.append(ttl)
    ...
    def min_choice_tile(self) -> Tile:
        ...
        for group in self.groups:
            for tile in group:
                if tile.value == UNKNOWN:
                    if len(tile.candidates) < small:
                        small = len(tile.candidates)
        for group in self.groups:
            for tile in group:
                if len(tile.candidates) == small and tile.value == UNKNOWN:
                    return tile
    ...
    def solve(self) -> bool:
        ...
        if not self.is_consistent():
            return False
        save = self.as_list()
        start = self.min_choice_tile()
        for val in start.candidates:
            start.set_value(val)
            if self.solve():
                return True
            else:
