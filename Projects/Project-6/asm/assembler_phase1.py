...
def transform(lines: list[str]) -> list[int]:
    ...
    for lnum in range(len(lines)):
        ...
        try:
            f = parse_line(line)
            if f["kind"] == AsmSrcKind.FULL:
                f = fix_optional_fields(f)
                if f["offset"] is None:
            ...
            elif f["kind"] == AsmSrcKind.DATA:
                data = f"{f['label']}: {f['opcode']} {f['value']}"
                transformed.append(data)
            elif f["kind"] == AsmSrcKind.MEMOP:
                ref = f["labelref"]
                mem_addr = labels[ref]
                pc_relative = mem_addr - address
                f = fix_optional_fields(f)
                mem = (f"{f['label']}   {f['opcode']}{f['predicate']} " +
                       f" {f['target']},r0,r15[{pc_relative}] #{ref} " +
                       f" {f['comment']}")
                transformed.append(mem)
            elif f["kind"] == AsmSrcKind.JUMP:
                ref = f["labelref"]
                mem_addr = labels[ref]
                pc_relative = mem_addr - address
                f = fix_optional_fields(f)
                jump = (f"{f['label']}   ADD{f['predicate']} " +
                        f" r15,r0,r15[{pc_relative}] " +
                        f" #{f['labelref']}")
                transformed.append(jump)
            else:
                log.debug(f"No instruction on line {lnum}: {line}")
                transformed.append(line)
            if f["kind"] != AsmSrcKind.COMMENT:
                address += 1
