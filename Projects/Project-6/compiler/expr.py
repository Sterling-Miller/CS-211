...
class Seq(Control):
    ...
    def gen(self, context: Context, target: str):
        """Store value of expression into variable several times"""
        left = self.left.gen(context, target)
        right = self.right.gen(context, target)
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        """if/else/fi"""
        loop_head = context.new_label("else")
        loop_exit = context.new_label("fi")

        self.cond.condjump(context, target, loop_head, jump_cond=False)
