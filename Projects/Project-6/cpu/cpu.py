...
class ALU(object):
    ...
    def exec(self, op: OpCode, in1: int, in2: int) -> tuple[int, CondFlag]:
        try:
            res = ALU.ALU_OPS[op](in1, in2)
            if res < 0:
                return res, CondFlag.M
            elif res == 0:
                return res, CondFlag.Z
            elif res > 0:
                return res, CondFlag.P
        except:
            return 0, CondFlag.V
...
class CPU(MVCListenable):
    ...
    def step(self):
        ...
        if predicate_res == CondFlag.NEVER:
            value = self.pc.get()
            value += 1
            self.pc.put(value)
        else:
            ...
            res, flag = self.alu.exec(instr.op, in1, in2)
            # Increment Counter
            value = self.pc.get()
            self.pc.put(value + 1)
